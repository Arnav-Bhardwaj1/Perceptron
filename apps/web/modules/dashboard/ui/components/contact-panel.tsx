...
export const ContactPanel = () => {
  ...
  // We fetch the contact session based on the conversation ID. If there is no conversation ID, we skip the query. Coversation Id was got from params, using it we got the conversation, and using the conversation we got the contact session. Contact session has all the metadata about the user, like user agent, location, etc. which we will display in the contact panel. You can see the table schema in the schema.ts file.
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col bg-background text-foreground">
      <div className="shrink-0 flex flex-col gap-y-4 p-4">
        <div className="flex items-center gap-x-2">
          <DicebearAvatar
            ...
            seed={contactSession._id} // seed means that the avatar will be generated based on the contact session ID, so each contact session will have a unique avatar
            size={42}
          />
        ...
        {contactSession.metadata && (
          ...
          >
            {accordionSections.map((section) => (
              ...
              >
                <AccordionTrigger
                  className="flex w-full flex-1 items-start justify-between gap-4 rounded-none bg-accent dark:bg-white/3 px-5 py-4 text-left font-medium text-sm outline-none transition-all hover:no-underline disabled:pointer-events-none disabled:opacity-50"
                >
