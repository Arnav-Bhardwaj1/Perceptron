...
(function() {
  ...
  if (!organizationId) {
    console.error('Perceptron Widget: data-organization-id attribute is required');
    return;
  }
  ...
  function render() {
    ...
    button.id = 'perceptron-widget-button';
    ...
    container.id = 'perceptron-widget-container';
  ...
  function hide() {
    if (container && button) {
      ...
      button.innerHTML = chatBubbleIcon;
      button.style.background = 'linear-gradient(135deg, #9a3412, #c2410c, #ea580c)';
    }
  }
  ...
  (window as any).PerceptronWidget = {
    init: reinit,
    show,
    hide,
    destroy
